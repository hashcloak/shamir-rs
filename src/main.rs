...
use shamir_secret_sharing::{generate_secret, get_shares_secret, interpolate};
...
fn get_shares_for_parties(secret: Fq, current_id: u64, other_parties: Vec<u64>) -> Vec<(Fq, Fq)> {
    ...
    match command.split_whitespace().collect::<Vec<&str>>().as_slice() {
        ...
        ["COMMUNICATE_SHARES"] => {
            ...
            for party in other_parties {
                let share: (Fq, Fq) = *shares
                    .iter()
                    .find(|(x, _)| *x == Fq::from(party.0))
                    .unwrap();
            ...
            // Store own share
            let own_share: (Fq, Fq) = *shares
                .iter()
                .find(|(x, _)| *x == Fq::from(current_id))
                .unwrap();
            shares_storage
                .lock()
                .await
        ...
        ["SUM_AND_DISTRIBUTE"] => {
            // Party has to sum their shares and send the result to the other parties
            // Calculate sum
        ...
        ["RECEIVE_SUM", id_party, sum] => {
            println!(
                "Received at party {} from party {}: SUM {}",
                current_id.to_string(),
                id_party,
                sum
            );
    ...
    loop {
        ...
        tokio::spawn(async move {
            mpc_party(
                ...
                current_party_id,
                [
                    (other_port_1.0, other_port_1.1),
                    (other_port_2.0, other_port_2.1),
                ],
                shares_clone,
                sums_clone,
            )
